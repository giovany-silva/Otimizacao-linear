...
{
    if((x_atual>=0&&y_atual>=0)&&(x_atual<largura&&y_atual<altura))
    {

if(maze[x_atual][y_atual]!='V')
{ 
if((maze[x_atual][y_atual]=='0'||maze[x_atual][y_atual]=='S'))
{if(maze[x_atual][y_atual]=='0')printf("(%d ,%d)",x_atual,y_atual);

  if((x_atual+1>=0&&x_atual+1<largura)&&(y_atual>=0&&y_atual<altura))
  { 
...
else if((x_atual-1>=0&&x_atual-1<largura)&&(y_atual>=0&&y_atual<altura))
  { 
     if(maze[x_atual-1][y_atual]=='E')
     {
      printf("(%d,%d)\n",x_atual-1,y_atual);
 exit(1);
 ...
 else if((x_atual>=0&&x_atual<largura)&&(y_atual-1>=0&&y_atual-1<altura))
  { 
  ...
  maze[x_atual][y_atual]='V';//isso eh para demarcar as posições vistidas  
